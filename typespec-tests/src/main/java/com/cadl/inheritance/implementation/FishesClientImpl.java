...
public final class FishesClientImpl implements FishesClient {
     ...
     */
    FishesClientImpl(InheritanceClientImpl client) {
        this.service = RestProxy.create(FishesService.class, client.getHttpPipeline(), client.getSerializerAdapter());
        this.client = client;
    }

    /**
     * The interface defining all the services for InheritanceClientFishes to be used by the proxy service to perform
     * REST calls.
     */
    @Host("{endpoint}")
    @ServiceInterface(name = "InheritanceClientFis")
